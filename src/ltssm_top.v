...
localparam [15:0]
    ...
    TMO_POLLING   = 16'd50000, // Polling.Active TS1/TS2 window (extended for simulation)
...
always @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
        ...
        if (state == ST_DETECT_ACTIVE) begin
            // FIX A: pipe_detect_lane (rx_receiver_detected_w) arrives 3 cycles
            // after the pipe_rx_status pulse due to rx_det FSM latency, so they
            // never overlap in the same cycle. Use pipe_rx_status==RXST_RECV_DET
            // alone — it is the authoritative PIPE signal for receiver detection.
            if (pipe_rx_status == RXST_RECV_DET) begin
                detect_done    <= 1'b1;
                detect_timeout <= 1'b0;
...
always @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
        ...
        if (state == ST_CFG_IDLE) begin
            // FIX B: Remove dll_up_req dependency (circular: LTSSM needs dll_up to
            // reach L0, but dll_up only asserts after L0 via DL_INIT + FC_INIT).
            // Per PCIe spec, LTSSM enters L0 when Config.Idle completes (TS2 agree
            // count reached). DL_Init (FC handshake) runs AFTER L0 entry.
            // Assert cfg_done immediately on ST_CFG_IDLE entry (TS2 agreed).
            cfg_done <= 1'b1;
        end else begin
            cfg_done    <= 1'b0;
            cfg_timeout <= 1'b0;
...
always @(*) begin
    ...
    case (state)
        ...
        ST_CFG_LINKWD_START: begin
            if (hot_reset_latch)
                next_state = ST_HOT_RESET;
            // FIX B2: advance on RECV_OK or on timer (negotiation complete)
            else if (pipe_rx_status == RXST_RECV_OK || timer_exp)
        ...
        ST_CFG_IDLE: begin
            if (hot_reset_latch)
                next_state = ST_HOT_RESET;
            // FIX B3: cfg_done now asserts immediately on CFG_IDLE entry.
            // dll_up_req is no longer a gate here (that was the circular dependency).
            else if (cfg_done || dll_up_req)
                next_state = ST_L0;
            // On timer expiry also go to L0 (link negotiation is done; DL_Init
            // happens after L0 entry per PCIe Base Spec 6.0 section 4.2.6.4).
            else if (cfg_timeout)
                next_state = ST_L0;
        end
...
always @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
        ...
        ltssm_state <= next_state;  // BUG-5 FIX: follow next_state not state
