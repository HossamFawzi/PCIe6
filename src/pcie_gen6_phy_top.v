...
module pcie_gen6_phy_top #(
    ...
    parameter SIM_BYPASS  = 0   // FIX-TC24: enables pipe_rxd direct-to-accumulator path
)(
    // ── Clocks & Resets ────────────────────────────────────────────────────
    ...
    input  wire         sim_acc_clear,       // SIM-ONLY: pulse high to clear RX accumulator
    ...
    output wire [15:0]  fec_syndrome_o,   // RS syndrome per flit (0 = clean) [15:0] legacy
    output wire [239:0] fec_syndrome_wide_o, // FIX-7: Full 30-symbol syndrome bus
...
always @(posedge clk or negedge phy_rst_n_comb) begin
    if (!phy_rst_n_comb) begin
        ...
        rx_fec_valid <= 1'b0;
        // FIX-TC24: sim_acc_clear provides synchronous accumulator reset
        // without relying on force/release of internal register (unreliable in iverilog).
        if (sim_acc_clear) begin
            rx_acc_cnt <= 4'd0;
            rx_acc_reg <= {2560{1'b0}};
        end else if ((SIM_BYPASS ? pipe_rx_valid : pam4_dec_valid_w) && gen6_mode_w) begin
            // FIX-TC24: Check-before-increment ordering:
            // cnt==9 means this is the 10th valid beat → fire flit and reset to 0.
            // Otherwise shift new word into accumulator and increment cnt.
            if (rx_acc_cnt == 4'd9) begin
                // 10th beat: the new word goes into acc_reg[255:0]; fec_data is
                // the upper 2348 bits of the 2560-bit accumulator (old[2303:0] + new[255:0]).
                rx_acc_reg   <= {rx_acc_reg[2303:0], (SIM_BYPASS ? pipe_rxd : pam4_dec_data_w)};
                rx_fec_data  <= rx_acc_reg[2559:212]; // upper 2348 bits of OLD reg (beat 0-8 + partial)
                rx_fec_valid <= 1'b1;
                rx_acc_cnt   <= 4'd0;
            end else begin
                rx_acc_reg   <= {rx_acc_reg[2303:0], (SIM_BYPASS ? pipe_rxd : pam4_dec_data_w)};
...
// FIX-7: Full 30-symbol syndrome for detailed UE diagnostics
// syndrome_w comes from fec_syndrome_calculator [255:0]; lower 240 bits = 30×8b symbols
assign fec_syndrome_wide_o   = syndrome_w[239:0];
