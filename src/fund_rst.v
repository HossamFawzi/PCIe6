...
module fund_rst (
    ...
    output reg         rst_done,      // All resets released (sticky level — cleared only by PERST#)
...
always @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
        ...
        // BUG-7 FIX: rst_done is now a sticky level; only cleared by PERST# or rst_n
        // PERST# asserted → immediate return to hold
        if (!perst_n) begin
            sys_rst_n <= 1'b0;
            dl_rst_n  <= 1'b0;
            phy_rst_n <= 1'b0;
            rst_done  <= 1'b0;   // clear on PERST#
            state     <= S_HOLD;
            timer     <= 16'd0;
        end else begin
            case (state)
