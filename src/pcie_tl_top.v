...
module pcie_tl_top #(
    // SIM_LOOPBACK=1: FC Init FSM auto-acknowledges (no real link partner needed)
...
// FIX-ORD: ordering_ok_gated prevents deadlock when ORD idles (ok=0) but
// still honours genuine stall signals from pcie_ordering_rob.
// Allow transmission when: (a) ORD says OK, OR (b) a request is queued
// AND no stall is signalled. This avoids the original combinatorial loop
// where ord_ok=0 → ARB stalls → ORD never advances → deadlock.
wire ordering_ok_gated = ordering_ok | (reqq_valid_out & ~ordering_stall);
...
// FIX-RX-RACE: Suppress dll_if path when the direct path is active.
// This prevents double-delivery when both paths fire on the same cycle.
wire dll_if_tlp_rx_valid_gated = dll_if_tlp_rx_valid_w & ~dll_tlp_rx_direct_valid;
assign dll_tlp_rx_valid = dll_tlp_rx_direct_valid | dll_if_tlp_rx_valid_gated;
...
// 4. FC Init FSM
tl_fc_init_fsm #(.SIM_LOOPBACK(SIM_LOOPBACK)) U_FC_INIT (
    .clk             (clk),
    .rst_n           (rst_n),
    .dll_up          (dll_up),
    .initfc_rx       (cr_update),
