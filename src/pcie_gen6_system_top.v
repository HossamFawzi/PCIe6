...
module pcie_gen6_system_top #(
    ...
    // SIM_LOOPBACK: propagated to tl_fc_init_fsm — enables self-loopback FC handshake in sim
    parameter SIM_LOOPBACK = `ifdef SIMULATION 1 `else 0 `endif,
    ...
    parameter SIM_BYPASS = `ifdef SIMULATION 1 `else 0 `endif
)(
    ...
    input  wire         sim_acc_clear,    // SIM-ONLY: clear RX accumulator in phy_top
...
wire [15:0]   dll_fec_syndrome_w;     // legacy 16-bit syndrome → DLL
wire [239:0]  dll_fec_syndrome_wide_w; // FIX-7: full 30-symbol syndrome
wire          dll_fec_corrected_w;     // corrected flag → DLL
...
pcie_gen6_phy_top #(
    ...
    .SIM_BYPASS (SIM_BYPASS)    // FIX-TC24: pipe_rxd→accumulator bypass
) u_phy_top (
    // Clocks & Resets
    .clk                    (clk),
    .clk_pipe               (clk_pipe),
    ...
    .fec_syndrome_wide_o    (dll_fec_syndrome_wide_w), // FIX-7
...
pcie_tl_top #(.SIM_LOOPBACK(SIM_LOOPBACK)) u_tl_top (
    .clk                    (clk),
    .rst_n                  (rst_n),
