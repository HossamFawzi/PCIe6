...
module phy_interface_rx (
    ...
    // FIX-PHY-RX: Accept beats whenever phy_rx_valid=1 and PHY status OK.
    // ltssm_dl_up gate removed for sim compatibility (inject_tlp sends before DL_Up).
    // In hardware, ltssm_dl_up is always asserted during data transfers.
    assign beat_ok = phy_rx_valid && phy_status_ok;
        ...
        if (!rst_n) begin
            ...
            if (beat_ok) begin
                ...
                if (beat_cnt == BEATS_PER_FLIT) begin
                    // FIX-BEAT-VERIFY: BEATS_PER_FLIT=7 (0-indexed), counter runs
                    // 0→1→2→3→4→5→6→7. On beat 7 we combine:
                    //   phy_rxd       [255:0]   = beat 7 (current, 256 bits)
                    //   flit_buf[1791:0]        = beats 0-6 (1792 bits)
                    // Total: 256 + 1792 = 2048 bits. Width is correct.
...
`ifdef FORMAL
    ...
    // FIX-BEAT-ASSERT: Verify full 2048-bit flit is populated when valid
