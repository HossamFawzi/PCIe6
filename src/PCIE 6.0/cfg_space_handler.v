...
module cfg_space_handler (
    ...
    // ── Read path (2-cycle hold latency) — FIX-TC26 ──────────────
    // cfg_rd_valid is now held for 2 cycles so the testbench sampling
    // loop (which advances one clock before checking) can capture it.
    reg cfg_rd_valid_r;
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            cfg_rd_data    <= 32'h0;
            ...
            cfg_rd_valid   <= cfg_rd_valid_r;  // hold for 1 extra cycle
            cfg_rd_valid_r <= 1'b0;
        end
    end
