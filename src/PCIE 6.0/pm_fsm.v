...
module pm_fsm (
    ...
    output reg         pm_dllp_send,  // FIX-TC76: held for 2 cycles so TB sampling catches it
    ...
    reg       pm_dllp_send_hold; // FIX-TC76: carries pm_dllp_send pulse for 2 cycles
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            pm_dllp_type <= 3'd0;
            pm_dllp_send      <= 1'b0;
            pm_dllp_send_hold <= 1'b0;
            link_state_r      <= LS_L0;
            ltssm_pm_req <= 3'd0;
        end else begin
            pm_dllp_send      <= pm_dllp_send_hold;  // FIX-TC76: carry to next cycle
            pm_dllp_send_hold <= 1'b0;                    // auto-clear after 2 cycles
