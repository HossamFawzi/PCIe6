// =============================================================
//  MODULE : aer_error_logger  [FIXED v2]
//  Fix 1  : VER-134 — removed mixed blocking/nonblocking assignments.
//  Fix 2  : aer_int PULSE — aer_int is now a true single-cycle pulse.
//           It asserts only when NEW status bits appear (i.e., when
//           new_status has bits set that were NOT already in aer_status).
//           This satisfies the SVA: aer_int |=> !aer_int
//  Method : Compare new_status against aer_status (previous registered
//           value). Only generate nxt_int when at least one new unmasked
//           bit is being added this cycle.
// =============================================================
module aer_error_logger (
    ...
    // Purely combinational temporaries — blocking only
    reg [31:0]  new_status;
    reg         any_error;
    reg [7:0]   msg_type;
    reg [31:0]  newly_set_bits;   // bits being set THIS cycle that weren't before

    // Registered-output next-values
    reg         nxt_int;
    reg         nxt_valid;
    reg [255:0] nxt_tlp;
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            // NEW: only bits that are NEWLY set this cycle (not already sticky)
            // This ensures aer_int is a true single-cycle pulse per SVA p_aer_int_pulse
            newly_set_bits = new_status & ~aer_status;

            // Generate interrupt and message TLP only for NEWLY set unmasked bits
            if (any_error && |(newly_set_bits & ~aer_mask)) begin
                case (err_severity)
