...
reg          sim_acc_clear_r; // FIX-TC24: drives sim_acc_clear DUT input
...
// FIX-FC-DEADLOCK: SIM_LOOPBACK=1 enables TL FC-Init auto-loopback so
// fc_init_done asserts without needing a real link partner.
pcie_gen6_system_top #(.NUM_LANES(16), .DATA_WIDTH(256), .SIM_BYPASS(1),
                       .SIM_LOOPBACK(1)) dut (
    ...
    .sim_acc_clear(sim_acc_clear_r), .pipe_rx_status(pipe_rx_status),
...
reg [5:0] ltssm_prev;
initial ltssm_prev = 6'h3F;  // FIX WARN-3: 6-bit max = 6'h3F (was 6'hFF, truncated)
always @(posedge clk)
...
initial begin
    ...
    // ?? Default values ???????????????????????????????????????????????????????
    rst_n=0; perst_n=0; power_good=0; clk_valid=0;
    pipe_rxd=0; pipe_rxdatak=0; pipe_rx_valid=0; sim_acc_clear_r=0;
    ...
    // FIX-TC24: Use sim_acc_clear synchronous pulse to reset accumulator.
    sim_acc_clear_r = 1'b1;
    @(posedge clk);   // accumulator clears synchronously on this edge
    sim_acc_clear_r = 1'b0;
    @(posedge clk);   // one settle cycle after clear

    // Inject exactly 10 beats: de-assert pipe_rx_valid BEFORE the final @clk
    // so the accumulator receives exactly 10 rising-edge samples.
    pipe_rx_elec_idle=0;
    // FIX-TC24: With check-before-increment RTL, firing happens when cnt==9 AND
    // pipe_rx_valid=1. The loop counts 0→8 (9 increments), leaving cnt=9 after beat 9.
    // The 10th loop iteration sets pipe_rx_valid after the @clk edge (NBA timing).
    // An 11th beat (with cnt=9 and pipe_rx_valid=1) triggers the flit fire → cnt=0.
    for(i=0; i<10; i=i+1) begin
        @(posedge clk);
        pipe_rx_valid=1;
        pipe_rxd = {$random,$random,$random,$random,$random,$random,$random,$random};
    end
    // 11th beat: cnt==9 → flit fires, cnt<=0
    @(posedge clk);
    pipe_rx_valid=0; pipe_rxd=0;
    // FIX-TC24: NBA race — at beat-10's posedge, cnt<=0 is a pending NBA.
    // Reading rx_acc_cnt at that SAME edge returns the pre-edge value (9).
    // One extra @clk lets the NBA commit so cnt=0 is fully visible in the TB.
    @(posedge clk);  // beat-10 fires: cnt<=0 as NBA (not yet committed to read)
    @(posedge clk);  // NBA now committed: cnt=0 is stable and readable
    ...
    begin : tc47_blk
        ...
        // FIX-TC47: force com_detect=0 by zeroing datak so FSM goes BLK_HUNT not SYM_HUNT
        // Also hold phy_rst_n_w=1 to prevent fund_rst from resetting block_lock FSM.
        force dut.u_phy_top.phy_rst_n_w     = 1'b1;   // FIX: prevent mid-test PHY reset
        force dut.u_phy_top.sync_hdr_rx_w  = 2'b01;   // valid sync header
        force dut.u_phy_top.rx_gear_valid_w = 1'b1;
        force dut.pipe_rxdatak              = 32'd0;   // com_detect=0: datak=0
        // Reset FSM to IDLE first so BLK_HUNT acquisition starts cleanly
        ...
        release dut.pipe_rxdatak;
        // Keep phy_rst_n_w forced through TC51 (released after TC51 block)
    ...
    // FIX-TC51: Build BLK_LOCK state naturally using LOCK_THRESH good headers,
    // then inject 3 bad headers (cnt→3) followed by 1 good header (cnt→0).
    // This avoids force/release timing fragility with state registers in iverilog.
    tc_num = 51;
    $display("\n[TC51] Block Lock FSM : good header in BLK_LOCK clears miss counter");
    begin : tc51_blk
        integer k51;
        // Step 1: Drive FSM to BLK_LOCK naturally (need LOCK_THRESH=4 good sync headers).
        // Force com_detect=0 (datak=0) so FSM goes BLK_HUNT not SYM_HUNT.
        // Force lock_tmr to max so timer never expires during test.
        // phy_rst_n_w already forced=1 from TC47 block (released after this TC).
        force dut.pipe_rxdatak              = 32'd0;    // com_detect=0
        force dut.u_phy_top.lock_tmr        = 16'hFFFF; // timer won't expire
        force dut.u_phy_top.rx_gear_valid_w = 1'b1;
        force dut.u_phy_top.sync_hdr_rx_w   = 2'b10;   // valid sync header
        // Reset FSM to IDLE first for deterministic start
        ...
        release dut.u_phy_top.u_blk_lock.cnt;
        repeat(8) @(posedge clk);
        $display("  [TC51-P1] state=%0d cnt=%0d block_lock=%b (expect BLK_LOCK=4)",
                 dut.u_phy_top.u_blk_lock.state,
                 dut.u_phy_top.u_blk_lock.cnt,
                 dut.u_phy_top.u_blk_lock.block_lock);   // enough for LOCK_THRESH=4: IDLE→BLK_HUNT(4)→BLK_LOCK

        // Step 2: 3 bad headers to push cnt to 3 (one below MISS_THRESH=4).
        // IMPORTANT: exactly 3 clocks — a 4th would trigger LOCK_LOST (cnt==4)!
        force dut.u_phy_top.sync_hdr_rx_w = 2'b11;     // invalid sync header
        repeat(3) @(posedge clk);  // cnt: 1→2→3 (stop here, MISS_THRESH=4)

        // Step 3: 1 good header → should reset cnt to 0, stay BLK_LOCK
        force dut.u_phy_top.sync_hdr_rx_w = 2'b10;     // valid sync header
        @(posedge clk);
        @(posedge clk);  // extra settle cycle
        release dut.u_phy_top.sync_hdr_rx_w;
        release dut.u_phy_top.rx_gear_valid_w;
        @(posedge clk);  // let combinatorial block_lock settle after release
        ...
        $display("  cnt=%0d  block_lock=%b",
                 dut.u_phy_top.u_blk_lock.cnt,
                 dut.u_phy_top.u_blk_lock.block_lock);
        release dut.u_phy_top.phy_rst_n_w;   // FIX: release PHY reset hold after TC51
        release dut.pipe_rxdatak;             // restore datak after TC51
        release dut.u_phy_top.lock_tmr;       // restore lock timer after TC51
    end
    clk_n(10);
    ...
    //        FIX-TC60: Reset reversed_r before test so prior LTSSM training state
    //        doesn't prevent the lane_map update (reversed_r=1 blocks re-latching).
    tc_num = 60;
    $display("\n[TC60] Lane Reversal : TS1 mirror match -> reversal_active=1, correct lane_map");
    begin : tc60_blk
        // FIX-TC60: Hold phy_rst_n_w=1 so fund_rst doesn't reset lane_rev mid-test.
        // Then force reversed_r=0 to clear any sticky LTSSM-training residue, 
        // and force reversal_det=0 so the detection path (not the LTSSM path) fires.
        force dut.u_phy_top.phy_rst_n_w = 1'b1;

        // Step 0: Clear sticky reversed_r and reversal_active
        force dut.u_phy_top.u_lane_rev.reversed_r      = 1'b0;
        force dut.u_phy_top.u_lane_rev.reversal_active  = 1'b0;
        @(posedge clk);
        release dut.u_phy_top.u_lane_rev.reversed_r;
        release dut.u_phy_top.u_lane_rev.reversal_active;

        // Step 1: Apply mirror-match stimulus
        // local_lane_id=3, mirror = MAX_LANE(15) - 3 = 12 → ts1_lane_num=12
        ...
        @(posedge clk);   // edge: reversal_indicated=1 → reversed_r<=1, lane_map<=12
        @(posedge clk);   // edge: reversal_active <= reversed_r|reversal_indicated = 1
        ...
        @(posedge clk);   // one more cycle so registered signals settle post-release

        release dut.u_phy_top.phy_rst_n_w;
