...
module polling_fsm #(
    ...
    // -------------------------------------------------------------------------
    // BUG-12 FIX: Decoded TS pulses from ts_det module (multi-lane aware).
    // These replace the unreliable raw rx_data[7:0] byte detection below.
    // ts_det.v already checks all active lanes and emits a clean 1-cycle pulse.
    // -------------------------------------------------------------------------
    input  wire        ts1_det_in,        // from ts_det: TS1 OS detected
    input  wire        ts2_det_in,        // from ts_det: TS2 OS detected
    ...
    // BUG-12 FIX: Use ts_det_in / ts2_det_in (multi-lane decoded pulses).
    // Raw byte detection kept only for COM and polarity-inversion check
    // which are single-symbol pattern matches still valid on raw PIPE data.
    wire rx_com_det  = rx_valid && rx_datak && (rx_data[7:0] == K28_5);
    wire rx_ts1_raw  = ts1_det_in;   // BUG-12 FIX: was rx_data[7:0]==TS1_IDENT
    wire rx_ts2_raw  = ts2_det_in;   // BUG-12 FIX: was rx_data[7:0]==TS2_IDENT
