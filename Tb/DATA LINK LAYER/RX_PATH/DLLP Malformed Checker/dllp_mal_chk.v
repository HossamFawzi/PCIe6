...
module dllp_mal_chk (
    ...
    output reg          dllp_type_ok,      // 1 = type is valid and fields legal
    output reg          dllp_mal_err,      // 1-cycle pulse: malformed detected
    output reg  [47:0]  dllp_clean,        // clean DLLP body (zero if malformed)
    output reg          dllp_clean_valid   // 1-cycle pulse: clean body is valid
);
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            // ?? Default: de-assert all 1-cycle pulses ????????????????????????
            dllp_type_ok    <= 1'b0;
            dllp_mal_err    <= 1'b0;
            dllp_clean      <= 48'd0;
            dllp_clean_valid<= 1'b0;

            // ?? Only process if CRC passed AND valid ?????????????????????????
            // We check BOTH dllp_crc_ok AND dllp_valid_in:
            // dllp_valid_in = DLLP_CRKC's dllp_valid_out (only high on CRC pass)
            // dllp_crc_ok   = explicit CRC pass flag
            // Both must be high ? belt and suspenders safety check
            if (dllp_valid_in && dllp_crc_ok) begin

                if (!any_mal) begin
                    // ?? PASS: type valid, fields legal ???????????????????????
                    dllp_type_ok    <= 1'b1;
                    dllp_clean      <= dllp_body;  // forward clean body
                    dllp_clean_valid<= 1'b1;
                    // dllp_mal_err stays 0

                end else begin
                    // ?? FAIL: malformed DLLP ?????????????????????????????????
                    // Per PCIe spec: malformed DLLPs are dropped silently.
                    // dllp_mal_err goes to DLL_ERR for error reporting.
                    dllp_mal_err    <= 1'b1;
                    dllp_type_ok    <= 1'b0;
                    dllp_clean      <= 48'd0;      // do NOT forward bad data
                    dllp_clean_valid<= 1'b0;
                end

            end
            // If not valid or CRC not ok: all outputs stay at default (0)
        end
    end
