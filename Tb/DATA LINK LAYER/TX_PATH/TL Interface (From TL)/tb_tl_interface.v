// =============================================================================
// Testbench: tb_tl_interface  (event-driven, no cycle-count assumptions)
// Module under test: tl_interface
//
// All checks use @(posedge dll_tlp_valid) or @(posedge dll_flit_valid) to
// wait for the DUT output to actually fire, then sample at the following
// negedge. This is fully portable across simulators.
// =============================================================================
`timescale 1ns/1ps
module tb_tl_interface;
    ...
    // ?? send_tlp ??????????????????????????????????????????????????????????????
    // Drives tlp_valid_in for exactly 1 clock cycle.
    // Then waits for dll_tlp_valid to go high (event-driven, no cycle count).
    // Settles at the negedge of the dll_tlp_valid pulse for stable sampling.
    // Returns with dll_tlp_valid=1 guaranteed.
    task send_tlp;
        input [1023:0] data;
        begin
            @(posedge clk);
            ...
            @(posedge clk);
            tlp_valid_in = 0;
            // Wait for dll_tlp_valid to actually go high - event-driven
            @(posedge dll_tlp_valid);
            @(negedge clk);  // settle to stable mid-cycle point
        end
    endtask
    ...
    initial begin
        ...
        // ?? T6: TLP suppressed in FLIT mode ???????????????????????????????????
        // Cannot use send_tlp here (it would wait forever on dll_tlp_valid).
        // Drive manually and confirm no output after 8 cycles.
        ...
        do_reset; flit_mode_en = 0;
        send_tlp(1024'hABCD);
        // send_tlp returns at negedge while dll_tlp_valid=1
        ...
        // ?? T14: FC update value propagation ??????????????????????????????????
        ...
        // ?? T19: Mode change doesn't corrupt latched data ?????????????????????
