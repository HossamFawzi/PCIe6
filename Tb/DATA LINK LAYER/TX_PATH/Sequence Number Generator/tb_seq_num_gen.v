// =============================================================================
// Testbench: tb_seq_num_gen  ? portable across all simulators
// Module under test: seq_num_gen
//
// dispatch_one task (simulator-agnostic):
//   1. Assert tlp_valid_in for exactly 1 clock cycle
//   2. Deassert tlp_valid_in
//   3. After deassert, seq_num holds the assigned value (registered output)
//   4. Sample seq_num at the negedge AFTER deassert posedge
//
// This works on all simulators because it reads a registered hold value,
// not a live combinational signal, eliminating all race conditions.
//
// Sequence number correctness is verified directly from captured last_seq
// values, NOT from seq_num while tv=1 (which may or may not be live).
//
// Self-calibration measures and reports seq_num hold behaviour at startup.
// =============================================================================
`timescale 1ns/1ps
module tb_seq_num_gen;
    ...
    // ?? CALIBRATION ???????????????????????????????????????????????????????????
    // Verify that our dispatch strategy (assert 1 cycle, deassert, read hold)
    // produces seq 0,1,2 for three consecutive dispatches.
    task calibrate;
        reg [11:0] s0, s1, s2;
        begin
            do_reset;
    ...
    // ?? dispatch_one ??????????????????????????????????????????????????????????
    // Assert tlp_valid_in for 1 clock cycle.
    // Deassert. Sample seq_num at negedge after deassert posedge.
    // seq_num holds the last assigned sequence number (registered).
    // Returns: last_seq = sequence number assigned by this dispatch.
    reg [11:0] last_seq;
    task dispatch_one;
        begin
            @(posedge clk); tlp_valid_in=1;   // assert ? counter advances
            @(posedge clk); tlp_valid_in=0;   // deassert
            @(negedge clk); last_seq=seq_num; // read hold value
            @(posedge clk);                   // idle gap before next dispatch
        end
    endtask
    ...
    initial begin
        ...
        // ?? T17: large batch (20 dispatches monotonic) ????????????????????????
