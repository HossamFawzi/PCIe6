...
module pcie_ordering_rob #(
    ...
    output reg         ordering_ok,
    output reg         ordering_stall,
    output reg         ordering_err
);
    ...
    // -------------------------------------------------------------------------
    // Registered outputs — FIX-2: register the pre-computed combinational
    // signals directly (not re-gated by req_valid here).
    // -------------------------------------------------------------------------
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ordering_ok    <= 1'b1;   // After reset: no pending TXs → ordering is OK
            ordering_stall <= 1'b0;
            ordering_err   <= 1'b0;
        end else begin
            ordering_ok    <= order_ok_comb;
            ordering_stall <= order_stall_comb;
            ordering_err   <= order_err_comb;
        end
    end
