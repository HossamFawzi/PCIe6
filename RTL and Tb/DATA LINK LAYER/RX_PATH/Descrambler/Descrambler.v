...
module Descrambler (
    ...
    // -------------------------------------------------------------------------
    // LFSR Sync-Error Detection
    // When a new seed arrives (e.g., from an Ordered Set on the RX path),
    // compare it with the current LFSR state. Mismatch → sync error.
    // In real HW, seed is extracted from received OS framing bits.
    // Here: if link_reset=0 and lfsr_seed != current state → flag error.
    // -------------------------------------------------------------------------
    wire seed_mismatch;
