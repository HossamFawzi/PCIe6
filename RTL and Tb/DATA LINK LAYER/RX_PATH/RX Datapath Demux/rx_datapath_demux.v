                ...
                if (rx_valid) begin
                    case (rx_data[7:0])   // framing byte is in LS byte of first word
                        STP: begin
                            ...
                            // Simplified: single-beat path for short TLPs.
                            tlp_rx       <= {rx_data[255:8], 32'b0};  // placeholder LCRC = 0
                            tlp_rx_valid <= 1'b1;
                        end
                        ...
                        default: begin
                            // Unrecognised framing — IDL / EDB / etc.
                            if (rx_data[7:0] != 8'hBC)  // 8'hBC = COM (skip silently)
                                rx_parse_err <= 1'b1;
                        end
                    endcase
                end
